...
import com.xkcoding.idea.plugins.yapi_helper.config.ChooseYApiProjectDialog;
import com.xkcoding.idea.plugins.yapi_helper.config.YApiHelperConfig;
import com.xkcoding.idea.plugins.yapi_helper.config.YApiProjectConfigInfo;
...
import com.xkcoding.idea.plugins.yapi_helper.curl.Assert;
...
import com.xkcoding.idea.plugins.yapi_helper.normal.MethodInfo;
import com.xkcoding.idea.plugins.yapi_helper.store.GlobalVariable;
import com.xkcoding.idea.plugins.yapi_helper.util.*;
import com.xkcoding.idea.plugins.yapi_helper.yapi.enums.RequestBodyTypeEnum;
import com.xkcoding.idea.plugins.yapi_helper.yapi.enums.RequestMethodEnum;
import com.xkcoding.idea.plugins.yapi_helper.yapi.enums.YApiInterfaceStatusEnum;
import com.xkcoding.idea.plugins.yapi_helper.yapi.model.*;
...
public class ApiGenerateAction extends AnAction {
    ...
    private void uploadHttpMethodsToYApi(Project project, PsiClass psiClass) throws IOException {
        ...
        if (StringUtils.isEmpty(config.projectId)) {
            ...
            String projectId = projectInfo != null && projectInfo.get_id() == null ?
                    Messages.showInputDialog("Input Project Id", "Project Id", Messages.getInformationIcon()) :
                    projectInfo.get_id().toString();
    ...
    protected void generateMarkdownForSelectedMethod(Project project, PsiMethod selectedMethod) throws IOException {
        ...
        if (generateSuccess) {
            NotificationUtil.infoNotify("YApi Helper doc success.", project);
        }
    }
    ...
    private boolean generateDocForClass(Project project, PsiClass psiClass, String dirPath) throws IOException {
        ...
        if (!notExist) {
            if (!config.overwrite) {
                int choose = Messages.showOkCancelDialog(fileName + ".md already exists,do you want to overwrite it?", "Overwrite Warning!",
                        "Yes", "No", Messages.getWarningIcon());
    ...
    private String buildFieldStr(FieldInfo info) {
        return getFieldName(info) + "|" + info.getTypeText() + "|" + getRequireStr(info.isRequire()) + "|" + getRange(info.getRange()) +
                "|" + info.getDesc() + "\n";
    }
